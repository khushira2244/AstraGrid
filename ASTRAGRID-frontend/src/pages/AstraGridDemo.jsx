          ...
          <div>
            <div className={styles.verdictBoard}>
              ...
              <div className={styles.vbBody}>
                <div className={styles.vbCase}>Case: <span>ASTRAGRID-001</span></div>
                {VB_ROWS.map(r => (
                ...
                {sec.cards.map(card => (
                  <div key={card.label} className={styles.evCard}>
                    ...
                    <div className={styles.evCardVal}>
                      {card.valPill
                        ? <span className={`${styles.vp} ${styles[card.valPill.cls]}`}>{card.valPill.text}</span>
                        : card.val}
                    </div>
                    <div className={styles.evCardSub} style={card.subStyle || {}}>{card.sub}</div>
                  </div>
                ))}
                {sec.extras.map((ex, i) => (
                  <div key={i} className={`${styles.findingCard} ${ex.red === false && !ex.amber ? styles.findingCardGreen : ''}`}
                    style={ex.amber ? { background: 'rgba(255,176,32,.06)', borderColor: 'rgba(255,176,32,.2)' } : {}}>
                    <div className={styles.fcLabel}>{ex.label}</div>
                    <div className={styles.fcVal}
                      style={ex.amber ? { color: 'var(--partial)' } : ex.red === false ? { color: 'var(--confirmed)' } : {}}>
                      {ex.val}
                    </div>
                  </div>
                ))}
        ...
        <div className={`${styles.wrap} ${styles.depGrid}`}>
          <div>
            ...
            <div className={styles.evChecked} style={{ marginBottom: 24 }}>
              <div className={styles.evCheck}>POWER_SUBSTATION_01 supplies power to WATER_PUMP_STATION_03</div>
            ...
            <div className={styles.evResult}>The dependency path makes the cascade hypothesis more evidence-supported than the direct-water-attack hypothesis.</div>
            <div className={styles.evTool} style={{ marginTop: 16 }}>
              <span>Tool: <b>get_dependency_summary</b></span>
              <span>Tool run: <b>TOOL-000006</b></span>
            </div>
          </div>
      ...
      <section className={styles.timelineSection}>
        <div className={styles.wrap}>
          <div className={styles.tlHeading}>
            <div className={`${styles.eyebrow} ${styles.eyebrowCenter}`}>How the Hypothesis Changed</div>
            <h2>Self-Correction <em>Timeline</em></h2>
            ...
            {TL_STEPS.map(step => (
              <div key={step.num} className={styles.tlStep}>
                ...
                <div className={styles.tlContent}>
                  ...
                  <span className={`${styles.tlBadge} ${styles[step.badgeCls]}`}>{step.badge}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className={styles.verdictSection}>
        <div className={`${styles.wrap} ${styles.vsInner}`}>
          <div className={styles.vsHeading}>
            <div className={`${styles.eyebrow} ${styles.eyebrowCenter}`}>Investigation Complete</div>
            <h2>Final <em>Verdict</em></h2>
          ...
          <div className={styles.verdictMain}>
            <div className={styles.vmTop}>
              ...
              <div className={styles.vmStats}>
                ...
                <div>
                  ...
                  <div className={`${styles.vmStatVal} ${styles.vmStatValGreen}`} style={{ fontSize: 22, paddingTop: 8 }}>Maintained</div>
                </div>
              </div>
            </div>
            ...
            <div className={styles.vmAttribution}>
              Country attribution is not claimed. Observed origin: 192.168.56.113 → 192.168.56.112. Infrastructure path: POWER_SUBSTATION_01 → WATER_PUMP_STATION_03.
            </div>
